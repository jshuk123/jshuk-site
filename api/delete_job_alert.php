...
try {
    ...
    // Delete the alert (this will also delete related logs due to CASCADE)
    $stmt = $pdo->prepare("DELETE FROM job_alerts WHERE id = ? AND user_id = ?");
    $stmt->execute([$alert_id, $user_id]);
    
    echo json_encode([
        'success' => true,
        'message' => "Job alert '{$alert['name']}' has been deleted"
    ]);
    
} catch (PDOException $e) {
    error_log("Delete job alert error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
