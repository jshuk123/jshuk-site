-- =====================================================
-- JShuk Subscription & Advertising Platform - SQL Setup
-- =====================================================
-- 
-- Developer Brief: Stripe Subscriptions & Advertising Integration
-- 
-- Purpose: This migration sets up all required database tables and columns to support:
-- - Stripe-based subscriptions
-- - Subscription plan management  
-- - Advertising slot purchases
-- - User subscription tracking and billing cycles
--
-- Use this if:
-- - You're implementing or debugging the Stripe checkout flow
-- - You need missing columns like stripe_customer_id, status, etc.
-- - You plan to offer paid advertising slots (e.g. featured listings)
--
-- Setup Instructions:
-- 1. BACKUP your existing database before applying this.
-- 2. Run this SQL in phpMyAdmin or via MySQL CLI.
-- 3. Check your application code to ensure it references the correct column/table names.
-- =====================================================

-- USERS TABLE: Add Stripe and subscription tracking fields
ALTER TABLE users
ADD COLUMN stripe_customer_id VARCHAR(255) DEFAULT NULL COMMENT 'Stripe customer ID for payment processing',
ADD COLUMN subscription_status VARCHAR(50) DEFAULT NULL COMMENT 'User subscription status',
ADD COLUMN subscription_plan_id INT DEFAULT NULL COMMENT 'Reference to subscription_plans table',
ADD COLUMN current_period_end DATETIME DEFAULT NULL COMMENT 'When current subscription period ends';

-- SUBSCRIPTION PLANS TABLE: Defines the plans users can choose from
...
-- USER SUBSCRIPTIONS TABLE: Track full Stripe subscription metadata
...
-- ADVERTISING SLOTS TABLE: Defines the ad packages available for users to buy
...
-- ADVERTISING BOOKINGS TABLE: Track which users have booked which slots
CREATE TABLE IF NOT EXISTS advertising_bookings (
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (slot_id) REFERENCES advertising_slots(id) ON DELETE CASCADE,
    INDEX idx_user_id (user_id),
...
-- Update existing users to have 'active' status if not already set
